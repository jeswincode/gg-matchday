  ...
  for (const field of ALLOWED_FIELDS) {
    ...
    if (field === "jerseyNumber") {
      ...
      if (!Number.isInteger(number) || number < 0 || number > 99) {
        throw new Error("Jersey number must be an integer from 0 to 99.");
      }
    ...
    if (field === "preferredFoot") {
      if (!["", "Left", "Right", "Both"].includes(String(value))) {
        throw new Error("Preferred foot is invalid.");
      }
...
function makeBeforeSnapshot(player, changes) {
  ...
  for (const field of Object.keys(changes)) {
    before[field] = player[field] ?? null;
  }
    ...
    try {
      ...
      if (!playerId || !mongoose.isValidObjectId(playerId)) {
        return res.status(400).json({
          message: "Choose a valid player profile.",
        });
      }
      ...
      const pending =
        await ProfileChangeRequest.findOne({
      ...
      const before = makeBeforeSnapshot(player, changes);
      ...
      const populated =
        await ProfileChangeRequest.findById(request._id)
          .populate("requestedBy", "name email profileImage role")
          .populate("player", "name profileImage height weight position preferredFoot jerseyNumber dateOfBirth bio");

      res.status(201).json(populated);
    } catch (error) {
      console.error(
        "Create profile change request error:",
        error
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      const requests =
        await ProfileChangeRequest.find({
          requestedBy: req.user._id,
        })
          .populate(
            "player",
            "name profileImage height weight position preferredFoot jerseyNumber dateOfBirth bio"
          )
          .sort({
            createdAt: -1,
          })
      ...
      console.error(
        "Load my profile requests error:",
        error
      );

      res.status(500).json({
        message:
          "Failed to load your profile requests.",
      });
    }
  }
);
...
// ==================================================
// ADMIN: LIST PROFILE REQUESTS
// GET /api/profile-requests/admin
// ==================================================
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "Load admin profile requests error:",
        error
      );
...
// ==================================================
// ADMIN: APPROVE
// POST /api/profile-requests/admin/:id/approve
// ==================================================
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!player) {
        return res.status(404).json({
          message: "Player profile no longer exists.",
        });
      }
      ...
      const populated =
        await ProfileChangeRequest.findById(
          request._id
        )
          .populate(
            "requestedBy",
      ...
      res.json({
        message: `${player.name}'s profile changes were approved.`,
        request: populated,
      });
    } catch (error) {
      console.error(
...
// ==================================================
// ADMIN: REJECT
// POST /api/profile-requests/admin/:id/reject
// ==================================================
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      request.reviewedBy = req.user._id;
      request.reviewedAt = new Date();
      request.rejectionReason = reason.slice(0, 300);
