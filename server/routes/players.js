...
// ==================================================
// UPDATE PREFERRED POSITIONS ONLY
// EDITOR / ADMIN ONLY
// ==================================================
  ...
  try {
    ...
    if (
      uniquePositions.some(
        (value) => !approvedPositions.includes(value)
      )
    ) {
      return res.status(400).json({
    ...
    res.status(500).json({
      message: "Failed to update preferred positions.",
    });
  }
});
  ...
  try {
    ...
    if (duplicate) {
      return res.status(409).json({
        message: "Another player already has this name.",
      });
    }
    ...
    if (preferredPositions !== undefined) {
      ...
      if (uniquePositions.some((value) => !approvedPositions.includes(value))) {
        return res.status(400).json({
          message: "Choose valid preferred positions.",
        });
      }
    ...
    player.preferredFoot = ["Left", "Right", "Both", ""].includes(preferredFoot)
      ? preferredFoot
      : "";
    player.jerseyNumber = jerseyNumber === "" || jerseyNumber === null || jerseyNumber === undefined
      ? null
      : Number(jerseyNumber);
    ...
    if (
      player.height !== null &&
      (!Number.isFinite(player.height) || player.height < 0 || player.height > 250)
    ) {
      return res.status(400).json({
    ...
    if (
      player.weight !== null &&
      (!Number.isFinite(player.weight) || player.weight < 0 || player.weight > 300)
    ) {
      return res.status(400).json({
    ...
    if (
      ...
      (!Number.isInteger(player.jerseyNumber) || player.jerseyNumber < 0 || player.jerseyNumber > 99)
    ) {
      return res.status(400).json({
...
router.delete("/:id", requireAuth, requireEditor, async (req, res) => {
  try {
    ...
    const hasMatchHistory = await Match.exists({
      $or: [
    ...
    if (hasMatchHistory) {
      return res.status(409).json({
        message:
          "This player has match history and cannot be deleted. Keep the profile instead.",
      });
    }
