import {hasRating,hasDefensivePerformance,id} from './statistics.js';
...
export function prepareMatch(body,previous=null){
 ...
 for(const p of body.participants){const old=previous?.participants.find(v=>id(v.player)===id(p.player));const legacy=old&&!hasRating(old.rating);if(!hasRating(p.rating)&&!(legacy&&(p.rating===null||p.rating===undefined)))throw new Error('Enter a rating from 0 to 10 for every participating player.');if(!hasDefensivePerformance(p.defensivePerformance))throw new Error('Enter defensive performance from 0 to 10 for every participating player.');p.defensivePerformance=Number(p.defensivePerformance);const ownGoals=Number(p.ownGoals??0);if(!Number.isInteger(ownGoals)||ownGoals<0||ownGoals>20)throw new Error('Own goals must be whole numbers from 0 to 20.');p.ownGoals=ownGoals;}
