...
function preservesLegacyStoredScore(body, previous) {
  ...
  if ((body.participants || []).some(participant => Number(participant.ownGoals || 0) > 0)) return false;
  ...
  return nextParticipants.every(participant => {
    const prior = previousParticipants.find(item => id(item.player) === id(participant.player));
