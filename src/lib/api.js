...
export async function api(path,{method='GET',body,signal,...options}={}){
 ...
 const task=(async()=>{const token=await auth?.currentUser?.getIdToken();const response=await fetch(`${API_URL}${path}`,{...options,method,signal,headers:{'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{})},...(body!==undefined?{body:typeof body==='string'?body:JSON.stringify(body)}:{})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||'Could not complete this request.');if(method==='GET')cache.set(key,{data,until:Date.now()+15000});else invalidate();return data;})().finally(()=>pending.delete(key));
